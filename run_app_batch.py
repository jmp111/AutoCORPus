...
if __name__ == "__main__":
    ...
    parser.add_argument('-a','--associated_data',type=bool, help="True if associated data is available, folder needs to be in the same directory with the base html file and has the same name")
    ...
    if associated_data:
        for file in filelist:
            ...
            associated_data_dir = os.path.join(base_dir,os.path.basename(file).replace('.html','')+'_associated_data')
            if os.path.isdir(associated_data_dir):
                command = "python ./extract_associated_data.py -a " + associated_data_dir + " -t " + os.path.join(target_dir,'associated_data')
            os.system(command)